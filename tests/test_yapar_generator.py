...
#     from parser.yapar_generator import generate_parser_code
...
def test_generated_file_contains_expected_snippets(tmp_path):
    yalp = tmp_path / "g.yalp"
    yalp.write_text("%token ID\n%%\nS : ID ;")

    lexer = tmp_path / "lexeitor.py"
    lexer.write_text("def scan(text): return [(text.strip(), text.strip())]")

    sys.path.insert(0, str(tmp_path))
    from parser.yapar_generator import generate_parser_code
    out = tmp_path / "parser_check.py"
    generate_parser_code(str(yalp), "lexeitor", str(out))

    content = out.read_text()
    assert "grammar = Grammar(" in content
    assert "ACTION = {" in content
    assert "GOTO = {" in content
    assert "def parse(text:" in content
